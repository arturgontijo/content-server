...
logging.basicConfig(level=10, format="%(asctime)s - [%(levelname)8s] "
                                     "- %(name)s - %(message)s")
log = logging.getLogger("run_example_async_service")


def main():
    parser = argparse.ArgumentParser(description="Run services")
    parser.add_argument("--no-daemon", action="store_false", dest="run_daemon",
                        help="do not start the daemon")
    parser.add_argument("--daemon-config",
                        dest="daemon_config",
                        help="Path of daemon configuration file, without "
                             "config it won't be started",
                        required=False
                        )
    parser.add_argument("--ssl", action="store_true", dest="run_ssl",
                        help="start the daemon with SSL")
    args = parser.parse_args()
    root_path = pathlib.Path(__file__).absolute().parent
    
    # All services modules go here
    service_modules = ["service.example_async_service"]
    
    # Call for all the services listed in service_modules
    all_p = start_all_services(root_path, service_modules, args.run_daemon,
                               args.daemon_config, args.run_ssl)
...
def start_service(cwd, service_module, run_daemon, daemon_config, run_ssl):
    ...
    def add_ssl_configs(conf):
        ...
        with open(conf, "r") as f:
            ...
            snetd_configs["ssl_cert"] = \
                "/etc/letsencrypt/live/bh.singularitynet.io/fullchain.pem"
            snetd_configs["ssl_key"] = \
                "/etc/letsencrypt/live/bh.singularitynet.io/privkey.pem"
    ...
    if run_daemon:
        if daemon_config:
            ...
            for idx, config_file in enumerate(
                    glob.glob("./snetd_configs/*.json")):
                if run_ssl:
